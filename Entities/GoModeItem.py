...
class GoModeItem(Item):
    def __init__(self, id, name, image, position, enable, opacity_disable, hint, background_glow, always_enable=False):
        ...
        Item.__init__(self, id=id, name=name, image=image, position=position, enable=enable,
                      opacity_disable=opacity_disable, hint=hint, always_enable=False)
        self.can_drag = False
    def update(self):
        Item.update(self)
        if self.enable:
            ...
            pos_x = x_image * -1
            pos_y = y_image * -1
            tempSurface = pygame.Surface(([self.background_glow.get_rect().w, self.background_glow.get_rect().h]),
                                         pygame.SRCALPHA, 32)
            ...
            self.rect = pygame.Rect(self.position[0] + pos_x, self.position[1] + pos_y,
                                    self.colored_image.get_rect().width,
